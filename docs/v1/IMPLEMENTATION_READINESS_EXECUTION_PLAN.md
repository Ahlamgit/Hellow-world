...
| BLOCKER-004 | Cloud infrastructure approval | Blueprint + sizing/cost framework ready; provider/budget/workload Pending | Fill workload; compare vendors via framework; approve budget/RPO/RTO | DevOps / Business | Portable deploy pattern (ADR-012/024) | Cloud provider, region, hosting, budget approved | **IN PREPARATION** |
...
**Artifacts:** [`infra/CLOUD_INFRASTRUCTURE_DECISION.md`](./infra/CLOUD_INFRASTRUCTURE_DECISION.md) · [`infra/CLOUD_SIZING_AND_COST_FRAMEWORK.md`](./infra/CLOUD_SIZING_AND_COST_FRAMEWORK.md)

**Rule:** Decide hosting **targets**. Do **not** modify approved architecture. Remain vendor-neutral until approved. Use sizing/cost framework for provider comparison (no invented prices).
