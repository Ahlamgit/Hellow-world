...
public interface IIdentityRepository
{
    ...
    Task<IReadOnlyList<(Guid Id, string Code, string NameEn, string Module)>> GetAllPermissionsAsync(CancellationToken cancellationToken = default);
    Task<IReadOnlyList<Guid>> GetRolePermissionIdsAsync(IReadOnlyList<Guid> roleIds, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<UserPermission>> GetUserPermissionOverridesAsync(Guid userId, CancellationToken cancellationToken = default);
    Task SetUserPermissionOverridesAsync(Guid userId, IReadOnlyList<(Guid PermissionId, bool IsGranted)> overrides, string? grantedBy, CancellationToken cancellationToken = default);
